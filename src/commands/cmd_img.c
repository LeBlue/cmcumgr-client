...
int cmd_img_run_image_upload(struct smp_transport *transport, struct mgmt_image_upload_req *req, struct mgmt_rc *rsp, upload_progress_fn cb)
{
    ...
    state.progress.size = req->image.img_sz;
    ...
    cnt = mgmt_create_image_upload_seg0_req(buf, sizeof(buf), req->image.img_sz, file_buf, req->image.hash, sizeof(file_buf));
    ...
    rc = mgmt_img_upload_decode_rsp(buf, sizeof(buf), &state.offs, &rsp->mgmt_rc);
    if (rc || rsp->mgmt_rc) {
    ...
    while (state.offs < req->image.img_sz) {
        ...
        cnt = mgmt_create_image_upload_segX_req(buf, sizeof(buf), state.offs, file_buf, sizeof(file_buf));
